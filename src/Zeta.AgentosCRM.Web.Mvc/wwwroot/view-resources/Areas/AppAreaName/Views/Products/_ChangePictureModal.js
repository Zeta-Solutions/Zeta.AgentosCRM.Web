(function ($) {
    ...
    app.modals.ChangeProfilePictureModal = function () {
        ...
        var _profileService = abp.services.app.productProfile;
        ...
        function saveInternal(input) {
            ...
            if (input.ProductId > 0) {
                _profileService.updateProfilePicture(input).done(function (data) {
                    if ($cropperJsApi) {
                        $cropperJsApi = null;
                    }
                    var productId = input.ProductId ?? 0;
                ...
                _profileService.insertProfilePictureForProduct(input).done(function (data) {
                    if ($cropperJsApi) {
                        $cropperJsApi = null;
                    }
                    var agentId = input.agentId ?? 0;
                    ...
                    var imageUrl = $.ajax({
                        url: abp.appPath + 'api/services/app/ProductProfile/GetProfilePictureByPictireId',
                        data: {
