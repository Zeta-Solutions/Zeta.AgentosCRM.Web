(function () {
    ...
    $(function () {
        ...
        var _$enquiriesTable = $('#EnquiriesTable');
        //console.log(_$enquiriesTable);
        var _enquiriesService = abp.services.app.cRMInquiries;
        ...
        var dataTable = _$enquiriesTable.DataTable({
            paging: true,
            serverSide: true,
            processing: true,
            listAction: {
                ajaxFunction: _enquiriesService.getAll,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        /* console.log(data);*/
                        debugger
                        var rowId = data.crmInquiry.id;
                        console.log(rowId);
                        var contaxtMenu = '<div class="context-menu" style="position: absolute;">' +
                            '<div><input type="checkbox" class="custom-checkbox" value="' + data.crmInquiry.email + '"></div>' +
                            '<input hidden class="phoneNo" value="' + data.crmInquiry.phoneNo + '"></div>' +
                            '<input hidden class="clientchangeassignee" value="' + data.crmInquiry.id + '"></div>' +
                            '</div>';
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        //let firstNameInitial = row.crmInquiry.firstName.charAt(0).toUpperCase();
                        //let lastNameInitial = row.crmInquiry.lastName.charAt(0).toUpperCase(); 
                        //let initials = `${firstNameInitial}${lastNameInitial}`;
                        //let fullName = `${row.crmInquiry.firstName} ${row.crmInquiry.lastName}`;
                            let firstNameInitial = row.crmInquiry.firstName ? row.crmInquiry.firstName.charAt(0).toUpperCase() : '';
                            let lastNameInitial = row.crmInquiry.lastName ? row.crmInquiry.lastName.charAt(0).toUpperCase() : '';
                            let initials = firstNameInitial && lastNameInitial ? `${firstNameInitial}${lastNameInitial}` : '';
                            let fullName = row.crmInquiry.firstName && row.crmInquiry.lastName ? `${row.crmInquiry.firstName} ${row.crmInquiry.lastName}` : '';
                            // Further processing or rendering
                       

                        let clientDetailUrl = `/AppAreaName/Clients/ClientProfileDetail?id=${row.crmInquiry.id}`;
                        let clientEmailComposeUrl = `/AppAreaName/Clients/ClientEmailCompose?id=${row.crmInquiry.id}`;
    ...
    <div class="d-flex align-items-center">
        ...
        <div class="d-flex flex-column">
            <a href="${clientDetailUrl}" class="fs-6 text-uppercase text-bold" title="${fullName}">
             ...
             <a href="#" class="EmailForm" data-id="${row.crmInquiry.id}">${row.crmInquiry.email}</a>
        </div>
    </div>
`;
                    },
                ...
                {
                    targets: 6,
                    data: "crmInquiry.visaExpiryDate",
                    name: "VisaExpiryDate",
                ...
                {
                    targets: 8,
                    data: 'crmInquiry.lastModificationTime',
                ...
                {
                    targets: 9,
                    data: 'crmInquiry.preferedInTake',
                    name: 'preferedInTake',

                    render: function (lastModificationTime) {
                        if (lastModificationTime) {
                            return moment(lastModificationTime).format('MMMM YYYY');
                        }
                        return "";
                    }
                },
                ...
                },
                {
                    ...
                    render: function (lastModificationTime) {
                        if (lastModificationTime) {
                            const modifiedDate = moment(lastModificationTime);
                            const currentDate = moment();
                            const daysDifference = currentDate.diff(modifiedDate, 'days');
                ...
                {
                    targets: 12,
                    data: 'crmInquiry.interstedService',
                    name: 'InterstedService',
                    render: function (data, type, row) {
                        // Check if the data is too long
                         // You can adjust the threshold length as needed
                        //return '<span class="hover-text" title="' + data + '">' + data + '</span>';
                        return '<span class="popuptext" id="myPopup">' + data + '</span>'
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        debugger
                        var rowId = full.crmInquiry.id;
                        var rowData = full.crmInquiry;
        ...
        $(document).ready(function () {
            $(document).on('mouseover', '.popuptext', function (e) {
                var popupText = $(this).text(); // Get the text from the hovered element
                var popup = $('<div class="popup">' + popupText + '</div>'); // Create the popup element
                popup.appendTo('body'); // Append the popup to the body

                // Position the popup relative to the mouse pointer
                popup.css({
                    top: e.pageY + 10, // Add 10 pixels offset from the mouse pointer vertically
                    left: e.pageX + 10 // Add 10 pixels offset from the mouse pointer horizontally
                });
            });

            $(document).on('mouseout', '.popuptext', function () {
                $('.popup').remove(); // Remove any existing popups when mouse moves away
            });
        });
        function getClients() {
        ...
        function tabgetClients() {
            ...
            // dataTable.ajax.reload();
            _enquiriesService.getAll(filters)
                .then(function (data) {
